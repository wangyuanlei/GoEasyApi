...
type Client struct {
	ClientID          string `gorm:"primary_key;type:varchar(255)"` // 客户端id
	ClientName        string `gorm:"type:varchar(255)"` // 客户端名称
	TokenValidation   TokenValidation `gorm:"embedded"` // token验证配置
	Encryption        Encryption `gorm:"embedded"` // 出参入参验证配置
	RedisCache        RedisCache `gorm:"embedded"` // redis缓存配置
...
type TokenValidation struct {
	...
	DataSource   string `gorm:"type:varchar(255)"` // 数据源名称
	TableName    string `gorm:"type:varchar(255)"` // 对应的表名
	TokenField   string `gorm:"type:varchar(255)"` // token对应的字段名
	UserIDField  string `gorm:"type:varchar(255)"` // 对应的user_id字段名
}

type Encryption struct {
	InputEnabled  bool `gorm:"default:false"` // 是否启用入参私钥加密
	OutputEnabled bool `gorm:"default:false"` // 是否启用出参私钥加密
	Algorithm    string `gorm:"type:varchar(255)"` // 使用的加密算法
